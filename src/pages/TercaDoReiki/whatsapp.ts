...
export const PRECO_BRL = 150;
export const DURACAO_MIN = 45;

// Terça do Reiki é recorrente (primeira terça de cada mês); os horários são exibidos como texto —
...
export const buildAgendarWhatsAppUrl = (horario?: string): string => {
    ...
    const msg = `Olá! Quero agendar a Terça do Reiki${quando} — sessão individual de ${DURACAO_MIN} minutos (R$ ${PRECO_BRL}).`;
    return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(msg)}`;
};

export const buildDuvidasWhatsAppUrl = (): string => {
    const msg = 'Olá! Tenho uma dúvida sobre a Terça do Reiki (sessão individual presencial, em Recife).';
    return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(msg)}`;
};
